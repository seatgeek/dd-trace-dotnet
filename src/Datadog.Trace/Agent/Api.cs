...
namespace Datadog.Trace.Agent
{
    internal class Api : IApi
    {
        ...
        public async Task<bool> SendTracesAsync(Action<Stream> writer, int count)
        {
            // retry up to 5 times with exponential back-off
            var retryLimit = 5;
            var retryCount = 1;
            ...
            while (true)
            {
                ...
                // Set additional headers
                request.AddHeader(AgentHttpHeaderNames.TraceCount, count.ToString());
                if (_frameworkDescription != null)
                {
                    request.AddHeader(AgentHttpHeaderNames.LanguageInterpreter, _frameworkDescription.Name);
                    request.AddHeader(AgentHttpHeaderNames.LanguageVersion, _frameworkDescription.ProductVersion);
                ...
                try
                {
                    success = await SendTracesAsync(writer, request).ConfigureAwait(false);
                }
                catch (Exception ex)
                {
#if DEBUG
        ...
        private async Task<bool> SendTracesAsync(Action<Stream> writer, IApiRequest request)
        {
            ...
            try
            {
                try
                {
                    ...
                    response = await request.PostAsync(writer).ConfigureAwait(false);
                }
                catch
                {
